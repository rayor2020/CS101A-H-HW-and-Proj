...
public:
    ...
    virtual int GetStartCooldown() const = 0; // 开局冷却时间
    ...
    void SetStartCooldown() { m_cooldownRemaining = GetStartCooldown(); }

    // 设置所属的 GameWorld
    void SetGameWorld(std::weak_ptr<GameWorld> world) { m_world = world; }

protected:
    Seed(ImageID imageID, int x, int y, LayerID layer, int width, int height)
