...
#include "pvz/Containers/SList.hpp"      // 自定义单向链表
...
public:
  ...
  /// @brief Increase the sunlight by @c inc. If @c inc is negative, the sunlight is actually decreased.
...
private:
  ...
  // 用自定义单向链表替换 std::list
  pvz::SList<std::unique_ptr<GameObject>> m_objects;
