...
本题满分总共30分，计算规则如下
min(基础功能实现(30 pts)+ 扩展内容实现 (15 pts) - 代码规范 (此处最多扣 20 pts), 30 pts)

### 基础功能实现 (30 pts)

以下的功能为加分项，功能应该正确实现没有 bug ，并且拥有合适的贴图与动画显示。

1. (3 pts) 点击向日葵种子，能成功在场地上种下向日葵并显示，植物种子有冷却时间。
2. (3 pts) 向日葵可以产生阳光，场地中也会随时间规律产生随机掉落阳光，阳光可以被收集用于种植植物。
3. (3 pts) 场地右侧会生成僵尸，僵尸会按照一定速度朝左侧移动，僵尸到达屏幕左侧后游戏失败。
4. (6 pts) 僵尸会啃咬植物造成伤害，植物血量与啃咬伤害数值设置合理，植物血量归零后消失。
5. (3 pts) 可以种植豌豆射手，豌豆射手可以按照一定攻击速度发射豌豆。
6. (6 pts) 射出的豌豆将会对第一个碰到的僵尸造成伤害，碰撞后豌豆将消失。
7. (6 pts) 成功实现铲子功能，当拿起铲子后若第一次点击到的是植物，那么将会移除该植物。
...
你需要满足下述代码规范，任何一项不满足你将会扣除对应的分数。此部分为扣分项，扣满 20% 为止。

1. **(20%)** 编译没有来自于你自己的代码的 warning 。请注意代码文件的include包括CMake文件的引用需要使用相对路径而非绝对路径，check时将在助教电脑上编译，因此你需要确保你的代码有“**发布**”的信心，你的代码不应该仅仅只能在本地运行，而是能够在不同的设备中编译运行。
2. (5%) 代码中不应手动管理内存，而是应该正确使用智能指针。
   - “正确使用智能指针”还包括**正确使用 `std::enable_shared_from_this` 相关设施**，例如，应该只在必要时调用 `shared_from_this()` ，而不是粗暴地将所有 `this` 都替换为 `shared_from_this()` 。
3. (5%) 至少添加一个新的cpp文件以及hpp文件。按类别把文件分在不同的子目录里，并为每个子目录都创建 `CMakeLists.txt` ，以 `add_subdirectory` 的方式添加进项目。
...
14. **(10%)** 你应当正确判断存储游戏内容对象所使用的数据结构（将实现写在Containers文件夹中），你的数据结构应该从CS101A的数据结构前5节课中挑选，并且自己独立实现该数据结构，**而非使用STL**。
## 提交
