...
RegularZombie::RegularZombie(int x, int y, std::shared_ptr<GameWorld> world)
    : Zombie(ImageID::REGULAR_ZOMBIE, x, y, LayerID::ZOMBIES, 20, 80, AnimID::WALK, 200, 3, 1)
{
    SetWorld(world);
...
BucketHeadZombie::BucketHeadZombie(int x, int y, std::shared_ptr<GameWorld> world)
    : Zombie(ImageID::BUCKET_HEAD_ZOMBIE, x, y, LayerID::ZOMBIES, 20, 80, AnimID::WALK, 1300, 3, 1)
{
    SetWorld(world);
    PlayAnimation(AnimID::WALK);
}
...
PoleVaultingZombie::PoleVaultingZombie(int x, int y, std::shared_ptr<GameWorld> world)
    : Zombie(ImageID::POLE_VAULTING_ZOMBIE, x, y, LayerID::ZOMBIES, 20, 80, AnimID::RUN, 340, 3, 1),
