...
namespace
{
    constexpr int kInitialSunCooldown = 180;
    constexpr int kSkySunInterval = 300;
    constexpr int kInitialZombieDelay = 1000;
    constexpr int kBaseZombieInterval = 600;
...
void GameWorld::Init()
{
    ...
    // 创建UI文本：显示阳光数量
    m_sunText = std::make_shared<TextBase>(50, WINDOW_HEIGHT - 78, std::to_string(GetSunCount()));
    m_sunText->SetColor(0.0, 0.0, 0.0);
    m_sunText->SetCentering(false);

    // 创建UI文本：显示波数
    m_waveText = std::make_shared<TextBase>(WINDOW_WIDTH - 160, 8, "Wave:" + std::to_string(GetWaveCount()));
    m_waveText->SetColor(0.0, 0.0, 0.0);
    ...
    // 创建种子按钮
    auto sunflowerSeed = std::make_shared<SunflowerSeed>();
    sunflowerSeed->SetGameWorld(shared_from_this());
    Add(sunflowerSeed);

    auto peashooterSeed = std::make_shared<PeashooterSeed>();
    peashooterSeed->SetGameWorld(shared_from_this());
    Add(peashooterSeed);

    auto wallnutSeed = std::make_shared<WallnutSeed>();
    wallnutSeed->SetGameWorld(shared_from_this());
    Add(wallnutSeed);

    auto cherryBombSeed = std::make_shared<CherryBombSeed>();
    cherryBombSeed->SetGameWorld(shared_from_this());
    Add(cherryBombSeed);

    auto repeaterSeed = std::make_shared<RepeaterSeed>();
...
void GameWorld::UpdateUiText()
{
    ...
    if (m_waveText)
    {
        m_waveText->SetText(std::to_string(m_WaveCount));
    }
}
...
{
    m_nextZombieTick = std::max(kMinimumZombieInterval, kBaseZombieInterval - 20 * m_WaveCount);
}
