...
int Book::cnt = 0;
/**
 * @brief Default constructor for Book class
 */
Book::Book()
    : bookId(++cnt), relatedBookCount(0), relatedBookId(nullptr),
     isbn(""), title(""), author(""), genre(""), publicationYear(0),
     isAvailable(true), borrowedBy(""), borrowDate(0), dueDate(0) {
}

/**
 * @brief Implement this parameterized constructor for Book class
...
Book::Book(std::size_t relatedbookcount,const std::string& isbn, const std::string& title,
           ...
           bookId(++cnt),relatedBookCount(relatedbookcount),
           relatedBookId(relatedBookCount > 0 ? new int[relatedBookCount]() : nullptr),
           isbn(isbn), title(title),author(author), genre(genre), publicationYear(year),
           isAvailable(true), borrowedBy(""), borrowDate(0), dueDate(0) {
}
// copy constructor
// one bookId is unique to one ISBN
Book::Book(const Book& other)
    : bookId(other.bookId), relatedBookCount(other.relatedBookCount),
      ...
      genre(other.genre),publicationYear(other.publicationYear),
      isAvailable(other.isAvailable),borrowedBy(other.borrowedBy),
...
Book::Book(Book&& other) noexcept
    : bookId(other.bookId),
      ...
      borrowedBy(std::move(other.borrowedBy)),
...
Book& Book::operator=(const Book& other) {
    ...
    if (other.relatedBookCount > 0) {
        newArr = new int[other.relatedBookCount];
        for (std::size_t i = 0; i < other.relatedBookCount; ++i) {
            newArr[i] = other.relatedBookId[i];
        }
    }
...
void Book::setRelatedBook(std::size_t count,const int* relatedBooks){
    ...
    int* newRelatedBookId = new int[count]();
    for (std::size_t i = 0; i < count; ++i) {
        newRelatedBookId[i] = relatedBooks[i];
    }
    delete[] relatedBookId;
    relatedBookId = newRelatedBookId;
...
/**
 * @brief Check if book is overdue
...
/**
 * @brief Get number of days overdue
...
/**
 * @brief Convert time_t to string representation
